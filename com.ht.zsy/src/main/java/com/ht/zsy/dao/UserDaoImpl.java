...
public class UserDaoImpl extends JdbcDaoSupport implements UserDao {
	...
	public int findUserId(String username) {
		String sql="select id from sys_users where username=?";
		return getJdbcTemplate().queryForInt(sql, new Object[]{username}, new int[]{java.sql.Types.VARCHAR});
	}
	/*	{
	"roleid1":{
		 ["roeid":"2","roelename":"admin"]
	},
	"roleid2":{ 
		["roeid":"3","rolename":"common user"]
	}

*/

	@Override
	public List<Map<String,Object>> findRoleId(String username) {
		String sql="select r.role,ur.role_id "
				+"from sys_users u,"
				+"sys_users_roles ur,"
				+"sys_roles r"
				+" where ur.user_id=u.id and u.username=? and ur.role_id=r.id";
			
		 return getJdbcTemplate().queryForList(sql,new Object[]{username},new int[]{java.sql.Types.VARCHAR});
	}
	@Override
	public List<Map<String, Object>> findPerssionId(String roleid) {
		
		String sql="select rp.role_id,p.permission,p.id as perssionId from sys_roles_permissions rp,sys_permissions p where rp.role_id=? and p.id=rp.permission_id";
		return getJdbcTemplate().queryForList(sql, new Object[]{roleid}, new int[]{java.sql.Types.VARCHAR});
	}
}
