        ...
        return this.each(function () {
            ...
            this.createHtml = function () {
                ...
                if (para.dragDrop) {
                    ...
                    //html += '						<div class="filePicker">选择照片</div>';
                    ...
                    html += '               <div class="filePicker">选择照片</div>';
                    html += '				<input id="fileImage" type="file" size="30" accept="image/jpg,image/jpeg,image/png" name="fileselect" ' + multiple + '>';
                    html += '				<div id="status_info" class="info">选中0张文件，共0B。</div>';
                    html += '				<div class="btns">';
                    //html += '					<div class="filePicker">继续选择</div>';
                    //html += '					<div class="upload_btn">开始上传</div>';
                    html += '					<input type="submit" value="开始上传"  form="uploadForm"  class="upload_btn" id="fileSubmit">';
                    html += '				</div>';
                    html += '			</div>';
                    html += '			<div id="preview" class="upload_preview"></div>';
                    html += '		</div>';
                    //html += '		<div class="upload_submit">';
                    //html += '			<button type="button" id="fileSubmit" class="upload_submit_btn">上传</button>';
                    //html += '		</div>';
                    ...
                    //html += '					<div class="upload_btn">开始上传</div>';
                    html += '					<input type="submit" value="开始上传" form="uploadForm" class="" id="fileSubmit">';
                    html += '				</div>';
                    html += '			</div>';
                    html += '			<div id="preview" class="upload_preview">';
                    html += '				<div class="add_upload">';
                    html += '					<a style="height:' + para.itemHeight + ';width:' + para.itemWidth + ';" title="选择照片" id="rapidAddImg" class="add_imgBox" href="javascript:void(0)">';
            ...
            this.funFilterEligibleFile = function (files) {
                ...
                for (var i = 0, file; file = files[i]; i++) {
                    size = (Math.round(file.size * 100 / (1024 * 1024)) / 100) * 1024;
                    if (size > 5120) {
                        //alert('您这个"' + file.name + '"文件大小过大');
                        alert('您这个"' + file.name + '"文件大小过大,不能超过5M!');
                    } else {
                        length++;
                        if (length > 10 || ZYFILE.fileNum>9) {
                            alert("您最多上传10张评论图片!");
                            break;
                        }
                        else {
                            if (!RegExp("\.(" + para.ImgType.join("|") + ")$", "i").test(file.name)) {
                                length--;
                                //alert("已过滤掉图片类型不是" + para.ImgType.join("，") + "的文件");
                            } else
                                // 在这里需要判断当前所有文件中
                                arrFiles.push(file);
                        }
            ...
            this.funDisposePreviewHtml = function (file, e) {
                ...
                if (file.type.indexOf("image") == 0) {
                    ...
                    html += '	<p id="uploadExist_' + file.index + '" class="file_exist">上传失败，已存在</p>';
            ...
            this.createCorePlug = function () {
                var params = {
                    ...
                    onDelete: function (file, files) {
                        ...
                        $(".hidValue input").remove("input[name='imgid']:eq(" + file.index + ")");
                    },
                    ...
                    onSuccess: function (file, response) {
                        ...
                        if (obj.code != "200") {
                            ...
                            $("#uploadExist_" + file.index).show();
                        } else {
                            $("#uploadProgress_" + file.index).hide();
                            $("#uploadSuccess_" + file.index).show();
                            var imgInfo = "<input type='text' value='" + obj.filename + "' name='imgid'>";
                            $(".hidValue").append(imgInfo);
                        }
